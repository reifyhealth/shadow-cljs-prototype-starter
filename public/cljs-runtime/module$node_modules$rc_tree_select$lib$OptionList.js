...
getEntityByValue(val))?val.key:null})},[checkedKeys]);_useKeyValueMap=React.useMemo(function(){return checkable?{checked:valueKeys,halfChecked:halfCheckedKeys}:null},[valueKeys,halfCheckedKeys,checkable]);React.useEffect(function(){if(open&&!multiple&&valueKeys.length){var _treeRef$current;null===(_treeRef$current=treeRef.current)||void 0===_treeRef$current?void 0:_treeRef$current.scrollTo({key:valueKeys[0]})}},[open]);var lowerSearchValue=String(searchValue).toLowerCase();treeDefaultExpandedKeys=
...
(0,_extends2.default)({ref:treeRef,focusable:!1,prefixCls:"".concat(prefixCls,"-tree"),treeData:memoOptions,height:height,itemHeight:itemHeight,virtual:virtual,multiple:multiple,icon:treeIcon,showIcon:showTreeIcon,switcherIcon:switcherIcon,showLine:treeLine,loadData:searchValue?null:loadData,motion:treeMotion,checkable:checkable,checkStrictly:!0,checkedKeys:_useKeyValueMap,selectedKeys:checkable?[]:valueKeys,defaultExpandAll:treeDefaultExpandAll},ref,{onActiveChange:_React$useState5,onSelect:onInternalSelect,
onCheck:onInternalSelect,onExpand:function(keys){setExpandedKeys(keys);setSearchExpandedKeys(keys);onTreeExpand&&onTreeExpand(keys)},onLoad:_React$useContext,filterTreeNode:function(treeNode){return lowerSearchValue?String(treeNode[treeNodeFilterProp]).toLowerCase().includes(lowerSearchValue):!1}})))});require.displayName="OptionList";exports.default=require}
